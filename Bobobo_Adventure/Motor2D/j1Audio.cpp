...
bool j1Audio::PreUpdate()
{
	...
	if (App->input->GetKey(SDL_SCANCODE_F7) == KEY_DOWN && mute == false) //Mute
	{
		mute = true;
		Mix_VolumeMusic(0);
		LOG("Mute ON!");
	}
	else if (App->input->GetKey(SDL_SCANCODE_F7) == KEY_DOWN && mute == true)
	{
		mute = false;
	...
	if (!mute)
	{
		if (App->input->GetKey(SDL_SCANCODE_KP_MINUS) == KEY_REPEAT) //Set DOWN Volume Music
		{
		...
		if (App->input->GetKey(SDL_SCANCODE_KP_PLUS) == KEY_REPEAT) //Set UP Volume Music
		{
